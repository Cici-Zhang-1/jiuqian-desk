 ...
 * @description  
 */
class Order extends CWDMS_Controller{
    private $_Module;
    private $_Controller;
    private $_Item;
    private $_Cookie;
		...
		if($this->form_validation->run($Item)){
		    ...
		    if(!!($this->Insert = $this->order_model->insert_order($Order))){
		        ...
		        if(!!($this->workflow->initialize('order',$this->Insert['oid']))){
		            ...
		            $this->Success .= $this->Insert['order_num'].'订单新增成功, 刷新后生效!';
		        }else{
		            $this->Failue = $this->workflow->get_failue();
		        }
		    }else{
		        $this->Failue .= isset($GLOBALS['error'])?is_array($GLOBALS['error'])?implode(',', $GLOBALS['error']):$GLOBALS['error']:'订单新增失败!';
		    }
		}else{
        ...
        if($this->form_validation->run($Run)){
            ...
            if(!!($Return = $this->order_model->is_editable($Selected))){
                ...
                if(!!($this->order_model->update_order($Post, $Selected))){
                    ...
                    $this->Success .= '订单修改成功, 刷新后生效!';
                }else{
                    $this->Failue .= isset($GLOBALS['error'])?is_array($GLOBALS['error'])?implode(',', $GLOBALS['error']):$GLOBALS['error']:'订单修改失败';
                }
            }else{
                $this->Failue .= isset($GLOBALS['error'])?is_array($GLOBALS['error'])?implode(',', $GLOBALS['error']):$GLOBALS['error']:'订单修改失败';
            }
