 ...
 * @des
 */
class Table_saw_workflow extends Workflow_abstract {
    private $_Source_id;
    ...
    public function table_saw(){
        if(!!($Workflow = $this->_get_next_workflow())){
            ...
            $this->_Workflow->store_message($Workflow['classify'].'已推台锯下料, 等待'.$Workflow['name']);
        }
    }
    ...
    public function re_produce(){
        ...
        $this->_Workflow->store_message('推台锯下料, 重新安排生产');
    }
    /**
     * 获取当前流程的下一流程
     */
