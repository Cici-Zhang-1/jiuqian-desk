...
class Menu_model extends Base_Model{
	...
	public function select($Field, $Con = array(), $Num = 0) {
	    ...
	    $Cache = $this->Cache . __FUNCTION__ . implode('_', $Con) . $Num;
     ...
     * @param $Ugid 用户组编号
     */
	public function select_allowed_by_ugid($Ugid) {
        $Item = $this->Item.__FUNCTION__;
        $Cache = $this->Cache.__FUNCTION__ . $Ugid;
        $Return = false;
        if(!($Return = $this->cache->get($Cache))){
            $Sql = $this->_unformat_as($Item, $this->Module);
	...
	public function select_by_uid($Uid) {
		$Item = $this->Item.__FUNCTION__;
		$Cache = $this->Cache.__FUNCTION__ . $Uid;
		$Return = false;
		if(!($Return = $this->cache->get($Cache))){
			$Sql = $this->_unformat_as($Item, $this->Module);
			$Query = $this->HostDb->select($Sql)->from('role_menu')
	...
	public function select_is_allowed_operation($Ugid, $Operation) {
        $Item = $this->Item.__FUNCTION__;
        $Cache = $this->Cache.__FUNCTION__ . $Ugid . $Operation;
        $Return = false;
        if(!($Return = $this->cache->get($Cache))){
            $Sql = $this->_unformat_as($Item, $this->Module);
            $Query = $this->HostDb->select($Sql)->from('role_menu')
