 ...
 * @description  
 */
class Menu extends CWDMS_Controller{
	private $_Module;
    private $_Controller;
    private $_Item ;
	public function __construct(){
		log_message('debug', 'Controller Priviledge/Menu Start!');
		parent::__construct();
		$this->load->model('priviledge/menu_model');
	...
	public function add(){
		...
		if($this->form_validation->run($Item)){
			...
			if(!!($Mid = $this->menu_model->insert($Post))){
			    $this->load->model('priviledge/role_menu_model');
		...
		if($this->form_validation->run($Item)){
			...
			if($Where !== false){
				...
				if (!!($this->menu_model->delete($Where)) && !!($this->role_menu_model->delete_by_mid($Where))) {
					$this->load->model('priviledge/func_model');
					$this->func_model->delete_by_mid($Where);
					$this->load->model('priviledge/card_model');
