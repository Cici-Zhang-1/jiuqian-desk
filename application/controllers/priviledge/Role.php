...
class Role extends CWDMS_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('priviledge/role_model');
    }
    ...
    public function add(){
        ...
        if($this->form_validation->run($Item)){
            ...
            if(!!($Id = $this->role_model->insert($Post))){
                $this->load->model('priviledge/usergroup_role_model');  // 在添加角色时，给管理组生成对应的角色
        ...
        if($this->form_validation->run($Item)){
            ...
            if($Where !== false && is_array($Where) && count($Where) > 0){
                if($this->role_model->delete($Where)){
                    $this->load->model('priviledge/role_menu_model');   // 删除角色时，要清除角色-菜单权限管理
                    $this->role_menu_model->delete_by_rid($Where);
                    $this->load->model('priviledge/role_func_model');   // 删除角色时，要清除角色-功能权限管理
                    $this->role_func_model->delete_by_rid($Where);
                    $this->load->model('priviledge/role_form_model');   // 删除角色时，要清除角色-表单权限管理
                    $this->role_form_model->delete_by_rid($Where);
                    $this->load->model('priviledge/role_card_model');   // 删除角色时，要清除角色-卡片权限管理
                    $this->role_card_model->delete_by_rid($Where);
                    $this->load->model('priviledge/role_element_model');   // 删除角色时，要清除角色-元素权限管理
